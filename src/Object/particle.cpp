...
int particle_t::FindUnusedParticle() {
...
//	this->data_block_size = {3,3,2,3};
	...
	this->data = new float[10000]; //adapt triangle strips into trangles to fit gpu_object(particleN* 2(traingles)*3(vertices)*3(x, y, z)

	
	this->vao->element_amount = MaxParticles * 2; //there are 2 triangles in each particles
	this->element = new GLuint[5000];
	for (int i = 0; i < 2*MaxParticles; ++i) { element[i] = i; }
...
void particle_t::draw(glm::mat4 modelMatrix){
	...
	for (int i = 0; i < new_particlesN; i++) {
		...
		float spread = 1.5f;
		glm::vec3 maindir = glm::vec3(0.0f, 25.0f, 0.0f);
		...
		ParticlesContainer[particleIndex].speed = maindir + (randomdir*0.3f) * spread;
	...
	for (int i = 0; i < MaxParticles; i++) {
		...
		if (p.life >= 0.0f) {
			...
			if (p.life > 0.0f) {
                if(p.speed.y<5){
                    if(!p.blasted){
                        p.speed = glm::vec3(p.blast_dir.x*2, -2, p.blast_dir.y*2);
                        //cout<< p.speed.x<<" "<<p.speed.y<<" "<<p.speed.z<<endl;
                        p.blasted = true;
                    }
                    p.speed+=glm::vec3(0, +0.000181f, 0);

                }
                else{
                    p.speed+=glm::vec3(0, -0.2081f, 0);
                }
//                cout<< p.speed.x<<" "<<p.speed.y<<" "<<p.speed.z<<endl;
                //cout<< p.life<<endl;
...
//            glm::vec4 v = glm::vec4 (g_vertex_buffer_data_small[3*j],g_vertex_buffer_data_small[3*j+1],p.pos.z + g_vertex_buffer_data_small[3*j+2], 1);
            ...
            glm::vec4 v = glm::vec4 (g_vertex_buffer_data[3*j],g_vertex_buffer_data[3*j+1], g_vertex_buffer_data[3*j+2], 1);
